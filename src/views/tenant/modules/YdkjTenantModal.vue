<template>
  <a-modal :title="title" :width="width" :visible="visible" :confirmLoading="confirmLoading" @ok="handleOk" @cancel="handleCancel" cancelText="关闭">
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        <a-form-item label="商户" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="[ 'tenant', validatorRules.tenant]" placeholder="请输入商户"></a-input>
        </a-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        url: {
          add: "/org.mall.modules/ydkjTenant/add",
          edit: "/org.mall.modules/ydkjTenant/edit",
        },
        userValue: undefined,
        userList: []
      }
    ...
    methods: {
      ...
      edit(record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model, 'tenant', 'company', 'companyLink', 'companyAddess', 'userId','userName','realName'))
        })
      },
      close() {
        this.$emit('close');
      ...
      popupCallback(row) {
        this.form.setFieldsValue(pick(row, 'tenant', 'company', 'companyLink', 'companyAddess', 'userId','userName','realName'))
      },
